...
function toRule(row) {
    ...
    const maskStrategy = row.maskStrategySelect.value;
...
function createRuleRow(initial) {
    ...
    "full keepFirstLast partialPercent".split(" ").forEach((strategy) => {
    ...
    row.typeSelect.addEventListener("change", () => {
        setRuleType(row, row.typeSelect.value);
    });
