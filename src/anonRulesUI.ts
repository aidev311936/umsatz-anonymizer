...
type RegexRuleInit = Partial<Extract<AnonRule, { type: "regex" }>>;

export interface AnonRulesUIController {
  setRules(rules: AnonRule[]): void;
  addRule(initial?: RegexRuleInit): void;
...
function createInput(labelText: string, type: string = "text"): { wrapper: HTMLLabelElement; input: HTMLInputElement } {
...
export function buildAnonRulesUI(container: HTMLElement, rules: AnonRule[]): AnonRulesUIController {
  ...
  function addRule(initial?: RegexRuleInit): void {
    const row: RuleRow = {
